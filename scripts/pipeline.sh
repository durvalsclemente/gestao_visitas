...
build_and_push() {
  ...
  docker build -f apps/web/Dockerfile -t "$REGISTRY/visitas-web:$TAG" \
    --build-arg VITE_API_BASE_URL=https://api.visitas.osc.app.br/api \
    --build-arg VITE_CENTRAL_URL=https://api.auth.osc.app.br \
...
smoke() {
  local CURL_API="curl -sk --resolve api.visitas.osc.app.br:443:127.0.0.1"
  local CURL_WEB="curl -sk --resolve visitas.osc.app.br:443:127.0.0.1"

  log "Smoke — api /health"
  for i in 1 2 3 4 5; do
    if $CURL_API -o /dev/null -w "%{http_code}" https://api.visitas.osc.app.br/health 2>/dev/null | grep -q 200; then
  ...
  CODE=$($CURL_API -o /dev/null -w "%{http_code}" https://api.visitas.osc.app.br/assistidos)
  ...
  if [ -n "$CODE_OAUTH" ]; then
    TOKEN=$(curl -sk --resolve api.auth.osc.app.br:443:127.0.0.1 -X POST https://api.auth.osc.app.br/api/v1/oauth/token -H 'Content-Type: application/json' \
    ...
    if [ -n "$JWE" ]; then
      ME=$($CURL_API -H "Authorization: Bearer $JWE" -o /dev/null -w "%{http_code}" https://api.visitas.osc.app.br/me)
